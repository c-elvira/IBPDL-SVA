...
namespace sva {
	 ...
	 */ 
	sva::DictAndCoef* svaDL(const Eigen::MatrixXd &Y, int nbIt, double lbd_reg1, double lbd_reg2);

	//std::tuple<Eigen::MatrixXd, Eigen::MatrixXd> svaDL_cythonInterface(const Eigen::Map<Eigen::MatrixXd> &Y, int nbIt, double lbd_reg1, double lbd_reg2);
	 ...
	 * @param e [description]
	 */
	void _update_D(const Eigen::MatrixXd &cmatYobs, sva::DictAndCoef &dictAndCoef, double varnoise);

	void _update_W_omp(const Eigen::MatrixXd &Y, sva::DictAndCoef &dictAndCoef, double lbd_reg1, double lbd_reg2, bool addAtom=true);
